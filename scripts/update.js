...
if (!hascnf(pkgConfig, 'configSchema', 'devel', 'properties', 'npmapi', 'default')) {
  console.error('NPM API URI missing')
  process.exit(1)
}
const npmapi = pkgConfig.configSchema.devel.properties.npmapi.default
if (!hascnf(pkgConfig, 'configSchema', 'devel', 'properties', 'docsBaseURL', 'default')) {
...
const npmapiPromise = new Promise((resolve) => {
  return request({json: true, url: npmapi}, function (error, res, attributes) {
    ...
    if (res.statusCode !== 200) {
      console.error(`Request checking A-Frame versions failed: ${res.statusCode}`)
      resolve(null)
    }
